...
export default function Sales() {
  ...
  const handleCartSubmit = async (
    ...
    for (const item of cart) {
      const saleData = {
        ...
        quantity: item.quantity, totalPrice: item.lineTotal,
        buyingPrice: item.product.buyingPrice, paymentMethod,
        date: new Date().toISOString().split("T")[0], shopId: currentShopId,
      ...
      logActivity({
        action: asDraft ? "draft_created" : "sale_created", category: "sale",
        details: asDraft
          ? `Draft: ${item.product.name} x${item.quantity} — ${formatTZS(item.lineTotal)}`
          : `${item.product.name} x${item.quantity} — ${formatTZS(item.lineTotal)}`,
        metadata: { productId: item.product.id, productName: item.product.name, quantity: item.quantity, totalPrice: item.lineTotal, paymentMethod, isDraft: asDraft },
      });
    }
    if (!asDraft) dispatch(fetchProducts(currentShopId));
    toast.success(asDraft ? `Draft ${cart.length} ${t("sales.draftsSaved")}` : `${t("sales.title")} ${cart.length} ${t("sales.recorded")}`);
    setDialogOpen(false);
  };
  ...
  const handleConfirmDraft = async (sale: import("@/types").Sale) => {
    try {
      ...
      toast.error(err?.message || t("sales.failedConfirm"));
    }
  };
  ...
  const handleDeleteDraft = async (sale: import("@/types").Sale) => {
    try {
      ...
      toast.error(err?.message || t("sales.failedDelete"));
    }
  };

  const closeReceipt = () => { setReceiptOpen(false); dispatch(clearLastCompletedSale()); };

  const filtered = completedSales.filter((s) => {
    const matchSearch = s.productName.toLowerCase().includes(search.toLowerCase()) || (s.customerName?.toLowerCase() || "").includes(search.toLowerCase());
    ...
    <div>
      <div className="page-header flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="page-description">{t("sales.subtitle")}</p>
        </div>
        {permissions.canAddSale && (
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button disabled={!currentShopId}><Plus className="h-4 w-4 mr-2" />{t("sales.newSale")}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>{t("sales.recordTitle")}</DialogTitle></DialogHeader>
              <SaleCart products={products} onSubmit={handleCartSubmit} canSaveDraft={permissions.canAddSale} />
            </DialogContent>
          </Dialog>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder={t("sales.searchPlaceholder")} className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-4">
            <TabsTrigger value="completed">
              {t("sales.completedTab")}
              {completedSales.length > 0 && <Badge variant="secondary" className="ml-2 h-5 min-w-[20px] text-xs">{completedSales.length}</Badge>}
            </TabsTrigger>
            <TabsTrigger value="drafts">
              {t("sales.draftsTab")}
              {draftSales.length > 0 && <Badge variant="destructive" className="ml-2 h-5 min-w-[20px] text-xs">{draftSales.length}</Badge>}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="completed">
            <div className="stat-card overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={7} className="py-8 text-center text-muted-foreground">{t("sales.noSales")}</td></tr>
                  ) : filtered.map((s, i) => (
              ...
              <div className="flex items-center justify-between border-t pt-3 mt-2 text-xs text-muted-foreground">
                <span>{t("products.total")}: {filtered.length} {t("sales.totalSales")}</span>
