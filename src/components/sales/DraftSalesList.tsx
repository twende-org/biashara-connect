...
export default function DraftSalesList({ drafts, canConfirm, canDelete, onConfirm, onDelete }: Props) {
  ...
  const handleConfirm = async (sale: Sale) => {
    ...
    try { await onConfirm(sale); } finally { setLoadingId(null); }
  };

  const handleDelete = async (sale: Sale) => {
    setLoadingId(sale.id);
    try { await onDelete(sale); } finally { setLoadingId(null); }
  };
  ...
  if (drafts.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">Hakuna draft za mauzo</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b text-left text-muted-foreground">
            ...
            <th className="pb-3 font-medium">Hali</th>
              ...
              <td className="py-3 text-right">
                <div className="flex items-center justify-end gap-1">
                  {canConfirm && (
                    ...
                    >
                      {loadingId === s.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <><CheckCircle2 className="h-3 w-3 mr-1" />Thibitisha</>}
                    </Button>
                  )}
                  {canDelete && (
                    <Button
