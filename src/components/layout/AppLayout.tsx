...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  const allNavItems = useMemo(() => [
    ...
    { label: t("nav.shops"), icon: Store, path: "/app/maduka" },
    { label: t("nav.products"), icon: Package, path: "/app/bidhaa" },
    { label: t("nav.sales"), icon: ShoppingCart, path: "/app/mauzo" },
    { label: t("nav.expenses"), icon: BarChart3, path: "/app/matumizi" },
    { label: t("nav.suppliers"), icon: Truck, path: "/app/wasambazaji" },
    { label: t("nav.users"), icon: Users, path: "/app/watumiaji" },
  ], [t]);
      ...
      <aside className={cn(
        ...
        <div className="px-3 py-3">
          <div className="relative">
            <button onClick={() => setShopDropdownOpen(!shopDropdownOpen)} className="flex w-full items-center gap-2 rounded-lg bg-sidebar-accent px-3 py-2.5 text-sm text-sidebar-accent-foreground hover:bg-sidebar-accent/80 transition-colors">
              <Store className="h-4 w-4 text-sidebar-muted" />
              <span className="flex-1 text-left truncate">{currentShop?.name || t("layout.selectShop")}</span>
              ...
              <div className="absolute left-0 right-0 top-full mt-1 rounded-lg border border-sidebar-border bg-sidebar shadow-lg z-10">
                {shops.map((shop) => (
                  <button key={shop.id} onClick={() => { dispatch(setCurrentShop(shop.id)); setShopDropdownOpen(false); }}
                    className={cn("flex w-full items-center gap-2 px-3 py-2.5 text-sm transition-colors first:rounded-t-lg last:rounded-b-lg",
                      shop.id === currentShopId ? "bg-sidebar-primary/20 text-sidebar-primary" : "text-sidebar-foreground hover:bg-sidebar-accent"
                    )}>
